...
export function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <div ref={ref} className={cn("relative", className)}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 rounded-md border border-border bg-card px-2.5 py-1.5 text-sm font-medium text-foreground hover:border-border/80 hover:bg-muted/50 transition-colors"
