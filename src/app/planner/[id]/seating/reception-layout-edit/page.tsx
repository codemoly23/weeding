...
function buildInitElements(): CeremonyElement[] {
  return [
    { id: "rt-0", kind: "table-round", x: 65, y: 60, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-1", kind: "table-round", x: 255, y: 60, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-2", kind: "table-round", x: 445, y: 60, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-3", kind: "table-round", x: 65, y: 360, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-4", kind: "table-round", x: 445, y: 360, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-5", kind: "table-round", x: 65, y: 640, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-6", kind: "table-round", x: 255, y: 640, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "rt-7", kind: "table-round", x: 445, y: 640, width: 80, height: 80, angle: 0, seats: 8, guestIds: Array(8).fill(null) },
    { id: "dance-floor", kind: "asset", assetType: "dance-area", x: 208, y: 325, width: 174, height: 150, angle: 0 },
    { id: "head-table", kind: "table-long", x: 195, y: 860, width: 200, height: 40, angle: 0, seats: 10, guestIds: Array(10).fill(null) },
  ];
}
...
function AssetIcon({ type, size = 40 }: { type: string; size?: number }) {
  ...
  switch (type) {
    ...
    case "firework": return <svg width={s} height={s} viewBox="0 0 40 40">{[0,36,72,108,144,180,216,252,288,324].map(d => <line key={d} x1="20" y1="20" x2={20 + 16 * Math.cos((d - 90) * Math.PI / 180)} y2={20 + 16 * Math.sin((d - 90) * Math.PI / 180)} stroke={["#ef4444","#f59e0b","#22C55E","#3b82f6","#8A6F3E"][Math.floor(d/72) % 5]} strokeWidth="2" strokeLinecap="round"/>)}</svg>;
    case "cake": return <svg width={s} height={s} viewBox="0 0 40 40"><rect x="8" y="22" width="24" height="12" rx="1" fill="#fce7f3" stroke="#E4A93B" strokeWidth="1.5"/><rect x="12" y="14" width="16" height="10" rx="1" fill="#fce7f3" stroke="#E4A93B" strokeWidth="1.2"/><line x1="14" y1="10" x2="14" y2="14" stroke="#f59e0b" strokeWidth="2"/><line x1="20" y1="8" x2="20" y2="14" stroke="#f59e0b" strokeWidth="2"/><line x1="26" y1="10" x2="26" y2="14" stroke="#f59e0b" strokeWidth="2"/></svg>;
...
export default function ReceptionLayoutEditPage() {
  ...
  const storageKey = `reception-layout-${projectId}`;

  const [elements, setElements] = useState<CeremonyElement[]>(() => {
    if (typeof window === "undefined") return buildInitElements();
    try {
      const saved = localStorage.getItem(`reception-layout-${projectId}`);
  ...
  function renderRightPanel() {
    ...
    return (
      <div className="flex flex-col divide-y divide-gray-100">
          ...
          {venueImage ? (
            <div className="space-y-2">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-[var(--color-error-text)]/30 px-2 py-1 text-xs text-[var(--color-error-text)] hover:bg-[var(--color-error-bg)]"
              >
        ...
        <PanelSection label="Paper"><p className="text-muted-foreground/70">A1 landscape 33.1 × 23.4 inch</p></PanelSection>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-card">
      ...
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-border/50 bg-card px-4 shadow-sm">
        <button onClick={() => router.push(`/planner/${projectId}/seating?tab=reception`)} className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted/30">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowTemplates(t => !t)} className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm text-foreground/80 hover:bg-muted/30">
      ...
      {showTemplates && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative flex flex-col bg-card rounded-xl shadow-2xl w-[560px] max-h-[80vh] overflow-hidden">
            <div className="flex shrink-0 items-center justify-between border-b border-border/50 px-5 py-3">
              <span className="text-sm font-semibold text-foreground">Reception Templates</span>
              <button onClick={() => setShowTemplates(false)} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/70 hover:bg-muted hover:text-foreground/80">
                <X className="h-4 w-4" />
              </button>
            </div>
            <p className="shrink-0 px-5 py-2 text-xs text-muted-foreground/70">Applying a template will replace current layout.</p>
            <div className="flex-1 overflow-y-auto px-5 py-3">
              <div className="grid grid-cols-2 gap-4">
                {RECEPTION_TEMPLATES.map(tmpl => (
                  <div key={tmpl.id} className="rounded-xl border border-border bg-muted/30 p-3 flex flex-col items-center gap-2">
                    ...
                    <button
                      ...
                      className="w-full rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-primary/90"
                    >
                      Use template
                    </button>
                  </div>
