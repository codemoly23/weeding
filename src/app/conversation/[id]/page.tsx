...
function ProjectContextRows({
  ...
  return (
    <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-xs text-primary">
      {project.eventDate && (
    ...
    return (
      <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">This link may be invalid or the conversation has been removed.</p>
          <Link href="/vendors" className="inline-flex items-center gap-1 mt-4 text-sm text-primary hover:underline">
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <header className="bg-card border-b border-border px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <Link href="/vendors" className="p-1.5 rounded text-muted-foreground/70 hover:text-foreground/80">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">{v.businessName}</p>
            <p className="text-xs text-muted-foreground">{v.city ? `${v.city}, ${v.country}` : v.country}</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto flex flex-col" style={{ height: "calc(100vh - 65px)" }}>
        ...
        {(conversation.project || conversation.inquiry) && (
          <div className="mx-4 mt-4 bg-primary/5 border border-primary/10 rounded-xl overflow-hidden">
            <button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {conversation.messages.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-sm text-muted-foreground/70">No messages yet. Your inquiry has been sent!</p>
            </div>
          ) : (
        ...
        <div className="bg-card border-t border-border px-4 py-3">
          {conversation.status === "SPAM" ? (
            <p className="text-sm text-center text-muted-foreground/70">This conversation has been closed by the vendor.</p>
          ) : (
          ...
          {sendError && <p className="text-xs text-[var(--color-error-text)] mt-1">{sendError}</p>}
