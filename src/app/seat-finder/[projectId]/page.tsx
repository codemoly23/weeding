...
export default function SeatFinderPage() {
  ...
  if (error || !data) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-accent/5 px-4 text-center">
        <QrCode className="mb-4 h-12 w-12 text-accent/50" />
        <h1 className="text-xl font-semibold text-foreground">Seating Not Available</h1>
        <p className="mt-2 text-sm text-muted-foreground">{error ?? "Seating information could not be found."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-accent/5 px-4 py-10">
      <div className="mx-auto max-w-lg">
        {/* Header */}
        <div className="mb-8 text-center">
          <Heart className="mx-auto mb-3 h-10 w-10 text-accent/60" />
          <h1 className="text-2xl font-bold text-foreground">{coupleNames}</h1>
          {eventDate && (
              ...
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all ${
                  selectedLayout === l.id
                    ...
                    : "bg-card text-foreground/80 border border-border hover:border-accent/30"
                }`}
              >
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-border bg-card pl-9 pr-4 py-3 text-sm shadow-sm focus:border-accent/30 focus:outline-none focus:ring-2 focus:ring-accent/10"
              />
            </div>
            <button
              type="submit"
              disabled={!query.trim()}
              className="rounded-2xl bg-accent px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-accent/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              Find
            </button>
          </div>
            ...
            {result ? (
              <div className="rounded-3xl bg-card p-6 shadow-lg">
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground/70">Your seat</p>
                    <p className="text-xl font-bold text-foreground">{result.table.name}</p>
                  </div>
                </div>

                <div className="mb-4 rounded-2xl bg-accent/5 px-4 py-3">
                  <p className="text-sm font-semibold text-accent">{result.guest.name}</p>
                    ...
                    <div className="space-y-1.5">
                      {result.table.guests
                        ...
                        .map((g) => (
                          ...
                          >
                            <div className="h-7 w-7 rounded-full bg-muted/70 flex items-center justify-center text-xs font-semibold text-foreground/80">
              ...
              <div className="rounded-3xl bg-card p-6 shadow-lg text-center">
                <Search className="mx-auto mb-3 h-10 w-10 text-muted-foreground/50" />
                <p className="font-semibold text-foreground">No seat found</p>
